...
* **Residual**
The <<ModelStructure>> element is extended with an additional element - <<Residual>> - and the optional dependencies can now include 
algebraic variables.

An FMU that follows this layered standard must expose all residuals with the <<Residual>>.

* **Invariants**
The <<ModelStructure>> element is extended with an additional element - <<Invariants>> - .
The dependency on differential states,inputs, and parameters can be optionally included  <<MN2018>>.
