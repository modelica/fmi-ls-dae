...
* **Invariant**
The <<ModelStructure>> element is extended with an additional element - <<Invariant>> - .
The dependency on differential states,inputs, algebraic variables, and parameters can be optionally included  <<MN2018>>.
...
|`Invariant`
|[[Invariant,`<Invariant>`]]
Ordered list of invariant equations.
Each `<Invariant>` indicate the FMU variable that computes an invariant of the FMU.
The invariant can ne scalar or vectorial.
The invariant are independent of `<Residual>` and the same invariant can also be mentionned in the `<Residual>` section.

The functional dependency is defined as: +
latexmath:[{0 := \mathbf{f}_{\mathit{inv}}(\mathbf{x}_c, \mathbf{u}_c, t, \mathbf{p})}]
|====
...
|`index`
|[[index-formulation]]
Optional attribute indicating the DAE index of the original constraint, i.e., how many times this particular equation needs to be differentiated in order to solve for a particular derivative.
...
Knowns latexmath:[{\mathbf{v}_{\mathit{known}}}] in *https://fmi-standard.org/docs/3.0.2/#ContinuousTimeMode[ContinuousTimeMode]* (ME) for `<<Formulation>>` elements are: +
...
**Case 2:**
The FMU additionally indicates that latexmath:[r_3] is a constraint of index 2, i.e., it needs to be be differentiated twice to obtain an index 1 system.
