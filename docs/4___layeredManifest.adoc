...
Two additional elements - <<AlgebraicVariables>> and <<ModelStructure>> - are included as direct children of the root element in the layered standard manifest file to describe the DAE formulation.
They are stored in the manifest rather than in the FMU's `modelDescription.xml`, as the FMI standard does not permit layered standards to extend that file.

.`fmiLayeredStandardManifest` child elements.
