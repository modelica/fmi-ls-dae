...
where latexmath:[z(t)] represents state and additional variables and latexmath:[t] is time.
...
with dynamic variables latexmath:[x(t)], algebraic variables latexmath:[y(t)] and time latexmath:[t].

Note that latexmath:[x(t)] is not necessarily the state variables, more on this in <<Perturbation Index, section 2.1.1>>.
...
with latexmath:[g] solvable for latexmath:[y], latexmath:[det \left( \frac{\partial}{\partial y}g\right) \neq 0].
...
can always be rewritten as a semi-explicit DAE by introducing a new algebraic variable latexmath:[x^{\prime}]
