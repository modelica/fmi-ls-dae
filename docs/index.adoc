...
where stem:[x] is position and stem:[v] is velocity.

The total energy stem:[H] (the Hamiltonian) is an invariant:
...
In the phase space of stem:[(x, v)], the analytical solution traces a perfect ellipse, forever staying on this "energy manifold."
...
* **Checking:** At regular intervals during the simulation, we calculate and monitor the value of the known invariants using the current numerical state. For example, we compute stem:[H(x_n, v_n)] and monitor its change from the initial value stem:[H(x_0, v_0)]. A significant deviation is a direct measure of the simulation's accumulated error with respect to the physics of the problem.
...
* **Correction (Projection):** Simply knowing there's drift isn't enough. For critical simulations, we use *geometric integrators* or *projection methods*.
  ...
  * **Projection Methods:** The correction is done during the integration. With standard numerical solvers (multi-step or single step), after a few steps or even after any step, the solution is "projected" back onto the correct invariant manifold. 
  For instance, the state stem:[(x_n, v_n)] is minimally adjusted to a new state stem:[(\tilde{x}_n, \tilde{v}_n)] such that stem:[H(\tilde{x}_n, \tilde{v}_n) = H(x_0, v_0)].  The Sundials numerical solver supports this useful feature.
...
Currently the only differential equations that can be supported by FMI is in the form. 
...
In order to be able to monitor invariants or constraints, 
extra invariant equations should be added to be monitored by the importer.
...
Invariants  equations (stem:[v=H(x)]) can be introduced in the current FMI-2.0/FMI-3.0 specification   exposed as local variables without breaking the backward compatibility.  
A new layared standard can be defined to designate the local invariant variables to be used by the numerical solver to rectify the solution at simulation time. 
